...
지금부터 **모노레포 도입기**와 **TBD (Trunk-based Development)**를 적용한 경험을 공유하려고 합니다.

모노레포 도입을 고민한 이유와 모노레포 도입과 운영, 그리고 모노레포 관리를 위한 브랜치 전략(TBD) 순서로 나눠보려고 합니다! 😁
...
따라서, 우선 성공적으로 yarn workspace를 도입한 후에 점진적으로 도입해 나갈 계획입니다.

## Build System

모노레포를 지원하는 도구 중에 **Nx**와 **TurboRepo**는 자신을 **Build System**이라고 소개하고 있습니다.
...
## local dependency로 바꾸기

# 마무리

모노레포를 성공적으로 완성하고, 모노레포 프로젝트는 TBD 전략을 통해 브랜치와 코드를 관리하고 있습니다.
