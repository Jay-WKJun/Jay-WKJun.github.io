...
Generic이란 용어는 type을 엄격하게 정해서 사용하는 Java 등의 언어에서는 매우 친숙한 용어이지만, type이란 것을 전혀 고려하지 않고 사용하는 JavaScript에서는 매우 생소한 용어입니다. 🤷‍♂️
...
DataStorage는 보통 범용적으로 쓰이기 때문에 any를 적용하는것이 좋겠죠. 하지만 type을 지정하고 싶다면 아래와 같이 상속을 해야할 것입니다.
...
하지만 이런 상속과정도 너무 번거롭고 코드의 양도 상속하는 수 만큼의 배로 늘어나게 됩니다...😫
...
위처럼 객체를 만들때, T의 타입을 정해주면 T 타입으로 할당된 모든 타입에 똑같이 적용이 됩니다.

또한 T타입이 정해졌기 때문에 return 되는 값도 T 타입이 되어 에디터가 타입을 알 수 있게 되므로 자동완성 기능이 가능하여 생산성이 향상된다는 장점도 있습니다!
...
저는 개인적으로 JavaScript에서 이런 code를 많이 썻습니다.
...
물론, type error입니다. ㅋㅋㅋ (저도 처음엔 많이 당황스러웠습니다...ㅋㅋㅋ)
