---
layout: post
title: 너와 나와 우리를 위한 클린코드 (feat. NextStep, TDD 클린코드 with React)
...
> 지금 이 코드를 리뷰해야하는 **지금의 너**와 내 코드를 보고 놀랄 **미래의 나**와 오랫동안 함께 이 코드를 관리해야 할 **우리 팀**을 위한 **클린코드**.
...
클린코드 책에선 **소프트웨어은 살아 숨쉬는 것**이며, 이를 건축과 비교하여 프로그래밍을 설명합니다.
...
**코드의 의도가 흐려지면, 리뷰어들은 코드를 읽는데 더 어려움**을 겪게 되겠죠,,,😱
